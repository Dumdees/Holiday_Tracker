 ...
 */
export function Table({ columns = [], rows = [], rowKey = 'id', onRowClick, selectable = false, selected, onSelectedChange, sort, onSortChange, sortRows: doSort = true, emptyState, stickyHeader = false, dense = false, rowClass, caption, ariaLabel, class: cls = '' }) {
          ...
          {list.length === 0 ? (
            ...
            return (
              ...
              >
                {selectable ? (
                  <td class="td-select">
                    <Checkbox checked={isSelected} onChange={(c) => toggleOne(k, c)} ariaLabel={isSelected ? 'Unselect this row' : 'Select this row'} />
                  </td>
                ) : null}
                {columns.map((col, ci) => (
